    ...
    <table class="table table-striped table-bordered" id="tablePeminjaman">
        ...
        while ($row = mysqli_fetch_assoc($result_peminjaman)) {
            echo "<tr>
                    ...
                    <td>{$row['status']}</td>
                    <td>
                        <a href='edit_peminjaman.php?id={$row['id']}' class='btn btn-warning'><i class='fa-solid fa-edit'></i> Edit</a> 
                        <a href='hapus_peminjaman.php?id={$row['id']}' class='btn btn-danger' onclick='return confirm(\"Yakin ingin menghapus data ini?\")'><i class='fa-solid fa-trash'></i> Hapus</a>
                    </td>
                </tr>";
            $no++;
        }
